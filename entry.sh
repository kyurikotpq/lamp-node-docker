...
mkdir -p /usr/share/webapps/ && cd /usr/share/webapps/ && \
    wget https://files.phpmyadmin.net/snapshots/phpMyAdmin-5.2+snapshot-all-languages.tar.gz > /dev/null 2>&1 && \
    tar -xzvf phpMyAdmin-5.2+snapshot-all-languages.tar.gz > /dev/null 2>&1 && \
    mv phpMyAdmin-5.2+snapshot-all-languages phpmyadmin && \
